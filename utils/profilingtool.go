...
func StartCPUProfile() {
	f, err := os.Create("d:\\cpu-" + strconv.Itoa(pid) + ".pprof")
	if err != nil {
		log.Fatal(err)
	}
...
func StopBlockProfile() {
	filename := "d:\\block-" + strconv.Itoa(pid) + ".pprof"
...
func DumpHeap() {
	filename := "d:\\heap-" + strconv.Itoa(pid) + "-" + strconv.Itoa(int(atomic.AddInt32(&heapProfileCounter, 1))) + ".pprof"
	f, err := os.Create(filename)
	if err != nil {
